...
def component_ref(**kwargs):
    return Subscript(value=kwargs["fld:OP0"], slice=kwargs["fld:OP1"])


def cond_expr(**kwargs):
    return If(test=kwargs["fld:OP0"], body=kwargs["fld:OP1"], orelse=None)
...
def eq_expr(**kwargs):
    return Compare(left=kwargs["fld:OP0"], ops=[Eq()], comparators=[kwargs["fld:OP1"]])
...
def identifier_node(**kwargs):
    if "fld:string" in kwargs:
        return NameWrapper(id=kwargs["fld:string"])
    else:
...
def modify_expr(**kwargs):
    return (Assign(targets=[kwargs["fld:OP0"]], value=kwargs["fld:OP1"]),)


def ne_expr(**kwargs):
    if "fld:OP0" in kwargs:
        return Compare(
            left=kwargs["fld:OP0"], ops=[NotEq()], comparators=[kwargs["fld:OP1"]]
        )
    else:
...
def truth_andif_expr(**kwargs):
    ...
    return BoolOp(values=[kwargs["fld:OP0"]], op=And(), comparators=[kwargs["fld:OP1"]])
